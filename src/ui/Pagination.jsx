...
function Pagination({ entryCount }) {
  const [searchParams, setSearchParams] = useSearchParams();
  const currentPage = !searchParams.get("page") ? 1 : Number(searchParams.get("page"));

  const pageCount = Math.ceil(entryCount / PAGE_SIZE);
  ...
  return (
    <StyledPagination>
      <P>
        ...
        <span>{currentPage === pageCount ? entryCount : currentPage * PAGE_SIZE}</span> of{" "}
        <span>{entryCount}</span> results
      </P>
      {pageCount > 1 && (
        <Buttons>
